...
import { useChildrenInfinite, ChildWithRelations } from "@/hooks/useChildrenInfinite";
...
export default function AdminChildren() {
  ...
  const {
    ...
    isLoading,
    refetch
  } = useChildrenInfinite(searchQuery, filterClass);

  const children = (data?.pages.flatMap((page) => page.data) || []) as ChildWithRelations[];

  // Parents Fetching (only when link dialog is open to save resources)
  ...
  // Helpers
  const getChildParents = (childId: string) => {
    const child = children.find(c => c.id === childId);
    if (!child || !child.parent_children) return [];

    return child.parent_children.map((pc) => ({
      id: pc.id,
      parent_id: pc.parent_id,
      child_id: childId,
      relationship: pc.relationship || "Responsável",
      profile: pc.profiles
    }));
  };

  const getAvailableParentsForChild = (childId: string) => {
    const currentParents = getChildParents(childId);
    const currentParentIds = currentParents.map(p => p.parent_id);
    return allParents.filter(p => !currentParentIds.includes(p.user_id));
  };
