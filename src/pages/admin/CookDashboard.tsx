...
export default function CookDashboard() {
  ...
  // mealTracking is now managed by useQuery
  ...
  const { data: weeklyMenu = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const menuByDay: Record<number, WeeklyMenu> = {};
      data?.forEach((item) => {
        // Force cast to compatible type since Supabase Json type is strict
        const jsonItem = item as unknown as WeeklyMenu;
        menuByDay[item.day_of_week] = jsonItem;
      });
      return menuByDay;
    },
    staleTime: 1000 * 60 * 5,
  ...
  const { data: mealTracking = {}, refetch: refetchTracking } = useQuery({
    queryKey: ["meal-tracking", today],
    queryFn: async () => {
      const { data } = await supabase
        .from("meal_tracking")
        .select("*")
    ...
    // Refresh often to see other cooks' updates
    refetchInterval: 30000,
  });

  const toggleMealMutation = useMutation({
    mutationFn: async ({ childId, mealKey, newValue }: { childId: string, mealKey: string, newValue: boolean }) => {
      ...
      if (existing) {
        const { error } = await supabase
          .from("meal_tracking")
          .update({ [dbFieldMap[mealKey]]: newValue })
          .eq("id", existing.id);
    ...
    onMutate: async ({ childId, mealKey, newValue }) => {
      await queryClient.cancelQueries({ queryKey: ["meal-tracking", today] });
      const previousTracking = queryClient.getQueryData<MealTracking>(["meal-tracking", today]);

      queryClient.setQueryData<MealTracking>(["meal-tracking", today], (old) => {
        const updated = { ...old };
        if (!updated[childId]) {
          updated[childId] = { breakfast: false, morningSnack: false, lunch: false, afternoonSnack: false, dinner: false };
        }
        updated[childId] = {
          ...updated[childId],
          [mealKey]: newValue,
    ...
    onError: (_err, _newVal, context) => {
      if (context?.previousTracking) {
        queryClient.setQueryData(["meal-tracking", today], context.previousTracking);
      }
      toast.error("Erro ao atualizar a refeição");
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["meal-tracking", today] });
    },
  });

  const toggleMeal = useCallback((childId: string, mealKey: string) => {
    const currentValue = mealTracking[childId]?.[mealKey as keyof MealTracking[string]] || false;
    toggleMealMutation.mutate({ childId, mealKey, newValue: !currentValue });
  }, [mealTracking, toggleMealMutation]);

  const markAllMeals = useCallback((mealKey: string, markAsServed = true) => {
    ...
    for (const child of filtered) {
      if (mealTracking[child.id]?.[mealKey as keyof MealTracking[string]] !== markAsServed) {
        toggleMealMutation.mutate({ childId: child.id, mealKey, newValue: markAsServed });
      }
    }
    toast.success(markAsServed ? "Todas as refeições marcadas!" : "Marcações removidas!");
  }, [children, mealTracking, selectedClass, toggleMealMutation]);
  ...
  const paginatedChildren = useMemo(() =>
    filteredChildren.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE),
            ...
            <TabsContent value="cardapio" className="mt-0 space-y-4">
              ...
              {currentMenu ? (
                <div className="space-y-3">
                  {mealTypes.map((meal) => {
                    const mealKey = meal.key.replace(/([A-Z])/g, '_$1').toLowerCase();
                    const mealValue = currentMenu[mealKey];

                    // Extract structured ingredients if available
                    const nutritionData = currentMenu.nutrition_data;
                    const ingredientsKey = `${mealKey}_ingredients`;
                    const ingredients = nutritionData?.[ingredientsKey];

                    // Extract quantity string if available (fallback)
                    const qtyKey = `${mealKey}_qty`;
                    const qtyString = currentMenu[qtyKey];
                    ...
                    return (
                      <Card key={meal.key} className={currentMeal === meal.key ? "border-orange-400 bg-orange-50/50" : ""}>
                        <CardContent className="p-3">
                          ...
                          <p className="text-sm mb-2">{mealValue}</p>

                          <Suspense fallback={<div className="h-4 w-20 animate-pulse bg-muted rounded" />}>
                            <MealIngredientsList
                              qtyString={qtyString}
            ...
            <TabsContent value="refeicoes" className="mt-0 space-y-4">
              ...
              <div className="flex items-center justify-between text-sm pt-2">
                <span className="text-muted-foreground ml-1">
                  Página {currentPage} de {Math.max(1, totalPages)} ({filteredChildren.length} crianças)
                </span>
                      ...
                      <div className="flex gap-1">
                        {mealTypes.map((meal) => (
                          <button
                            ...
                            className={`p-1.5 rounded transition-colors ${mealTracking[child.id]?.[meal.key as keyof MealTracking[string]]
