...
export function ParentAgendaView({ childId, childName }: ParentAgendaViewProps) {
  ...
  // Fetch daily record and set up realtime subscription
  useEffect(() => {
    let isMounted = true;
    ...
    channel = supabase
      .channel(`daily_records_${childId}_${dateStr}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        (payload) => {
          ...
          if (newRecord && newRecord.record_date === dateStr) {
            setRecord(newRecord);
            // We don't overwrite parentNotes here to prevent destroying what they might be typing
