...
export function TeacherParentChat() {
  ...
  const handleSend = async (content: string) => {
    ...
    if (!error) {
      // Fetch parents to send push notification
      const { data: registrations } = await supabase
        .from("child_registrations")
        .select("parent_id")
        .eq("status", "approved")
        .ilike("first_name", selectedChild.full_name.split(" ")[0]); // Simplificação para achar o registro
        ...
        if (parentIds.length > 0) {
          supabase.functions.invoke("send-push-notification", {
            body: {
              ...
              body: `A professora enviou uma nova mensagem para ${selectedChild.full_name.split(" ")[0]}.`,
              url: "/painel/chat",
              tag: "chat_school",
            },
  ...
  return (
    <div className="flex h-[450px] md:h-[500px] border rounded-lg overflow-hidden bg-background">
      ...
      <div
        className={`w-full md:w-80 border-r flex flex-col ${selectedChild ? "hidden md:flex" : "flex"
          }`}
      >
        {/* Header */}
            ...
            <div className="divide-y">
              {filteredChildren.map((child) => (
                <button
                  ...
                  className={`w-full p-3 text-left hover:bg-muted/50 transition-colors ${selectedChild?.id === child.id ? "bg-muted" : ""
                    }`}
                >
      ...
      {/* Chat Area */}
      <div
        className={`flex-1 flex flex-col ${!selectedChild ? "hidden md:flex" : "flex"
          }`}
      >
        {selectedChild ? (
          <>
