...
serve(async (req) => {
  ...
  try {
    ...
    // --- AUTH CHECK: only staff can send push notifications ---
    ...
    // Verify caller is staff (admin, teacher, etc.)
    const serviceClient = createClient(supabaseUrl, supabaseServiceKey);
    const { data: isStaff } = await serviceClient.rpc("is_staff", { _user_id: callerId });
    if (!isStaff) {
      return new Response(
        JSON.stringify({ error: "Forbidden: only staff can send notifications" }),
