...
// Logic moved to database and centralized function
// getContractValue will be replaced by direct DB query in the handler
      ...
      if (existingSubscription) {
        ...
        try {
          ...
          if (parentProfile) {
            ...
            // Calculate subscription value from DB logic
            let subscriptionValue = 0;

            // 1. Determine suggested class type from DB function
            let suggestedClassType = contract.class_type; // Default to contract value

            // If we have birth date, we can verify/refine the class type using the centralized logic
            // (Only if the contract class_type is generic 'maternal' or we want to be strict)
            if (childBirthDate) {
              // Call the centralized RPC function
              // Note: This requires the function to be accessible by the service role
              const { data: refinedClass } = await supabase.rpc('get_suggested_class_type', {
                birth_date: childBirthDate
              });
              if (refinedClass) {
                suggestedClassType = refinedClass;
              }
            }

            // 2. Fetch price from plans table
            if (suggestedClassType && contract.plan_type) {
              const { data: planData } = await supabase
                .from('plans')
                .select('price')
                .eq('class_type', suggestedClassType)
                .eq('plan_type', contract.plan_type)
                .single();

              if (planData) {
                subscriptionValue = planData.price;
              } else {
                console.error(`Plan not found for ${suggestedClassType} - ${contract.plan_type}`);
              }
            }
